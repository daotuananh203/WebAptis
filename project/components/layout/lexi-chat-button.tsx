...
export function LexiChatButton() {
  ...
  if (!isVisible) {
    return (
      <button
        ...
        aria-label="Mở Cố vấn AI Lexi"
        className="fixed bottom-20 sm:bottom-6 right-3 sm:right-6 z-30 p-3 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-400 text-white shadow-lg shadow-emerald-500/25 hover:scale-105 transition-transform cursor-pointer"
      >
  ...
  return (
    <div className="fixed bottom-20 sm:bottom-6 right-3 sm:right-6 z-30 flex items-center gap-2 group animate-in fade-in slide-in-from-bottom-4 max-w-[calc(100vw-1.5rem)]">
      {/* Floating Lexi CTA Card */}
      <Link
        href="/coach"
        className="flex min-w-0 items-center gap-3 pl-3.5 pr-4 py-2.5 rounded-2xl bg-[#141419] border border-[#242430] hover:border-emerald-500/50 shadow-xl shadow-black/60 transition-all hover:scale-[1.02] cursor-pointer"
      >
        {/* Robot Avatar */}
